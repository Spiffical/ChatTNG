...
class DialogMatcher:
    ...
    def match_dialog(self) -> List[Dict]:
        # Create a partial function with the instance method
        worker_func = functools.partial(self._match_single_segment)
        
        # Use multiprocessing to parallelize the matching
        with Pool() as pool:
            results = list(tqdm(
                pool.imap(worker_func, self.script_segments),
